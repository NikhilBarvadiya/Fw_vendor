...
class RequestAddressEditController extends GetxController {
  ...
  onSubmit(context) async {
    ...
    stylishDialog(
      ...
      onPressed: () {
        Get.back();
        if (Get.isRegistered<RequestAddressEditController>()) {
          Get.delete<RequestAddressEditController>();
        }
        if (Get.isRegistered<RequestAddressControler>()) {
          Get.delete<RequestAddressControler>();
        }
        Get.offNamed(AppRoutes.requestAddressScreen);
        update();
      },
    );
    update();
